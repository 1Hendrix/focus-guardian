...
The interesting part was not the blocking itself. It was making that blocking
...
- **Survives a macOS upgrade.** Upgrades silently delete system
  ...
  repairs it. That solves the bootstrap paradox: the thing that would
  restore the daemon was itself removed when the daemon was.
...
- When the current time falls in a window, writes the matching blocklist
  into `/etc/hosts`, writing both an IPv4 (`127.0.0.1`) and an IPv6 (`::1`)
  entry per domain so browsers cannot slip past the block over IPv6.
...
| `schedule.conf` | Your windows. Pure config, with nothing hardcoded. |
